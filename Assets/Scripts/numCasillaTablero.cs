...
public class numCasillaTablero : MonoBehaviour
{
    ...
    {
        if (this.GetEstaIluminada() == 0 && IniciaPartida.estaPausado == 0)
        {
            // Sonido movimiento incorrecto
            this.GetComponent<numCasillaTablero>().SonidoMovimientoIncorrecto();
        }
        if (this.GetEstaIluminada() == 1 && IniciaPartida.estaPausado == 0)
        {
            this.MueveFicha();
        }
    }
    ...
    // Activar sonido
    public void SonidoMovimientoIncorrecto()
    {
        audioSource.PlayOneShot(movimientoIncorrecto);
    }
